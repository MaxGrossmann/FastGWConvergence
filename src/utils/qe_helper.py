...
def qe_get_cutoff(structure):
    ...
    for elem in structure.types_of_species:
        if elem.symbol in ht_cutoff_SG15.data:
            pw_cutoff.append(ht_cutoff_SG15.data[elem.symbol])
        else:
            raise Exception(
                f"""
                QUITTING: No suggested cutoff is given for {elem.symbol} and SG15-PPs! 
                This means that is pseudopotential is not tested for this element!
                """
            )
...
def qe_get_eigenvalues(xml_path, num_elec):
    ...
    Also obtains the vbm and cbm. This only works for insulators and
    semiconductors! (only used in yambo workflows ...)
    INPUT:
        xml_path:       Path of the xml-output of a pw.x calculation
        num_elec:       Number of electrons in the cell
    OUTPUT:
        num_kpt:        number of k-points
    ...
    return num_kpt, eigenvalues, vbm, cbm
